...
add_filter( 'body_class', 'emoson_body_classes' );
...
add_filter( 'post_class', 'emoson_post_classes', 10, 3 );
...
add_action( 'wp_head', 'emoson_pingback_header' );
...
add_action( 'wp_footer', 'emoson_supports_js' );
...
add_filter( 'comment_form_defaults', 'emoson_comment_form_defaults' );
...
function emoson_can_show_post_thumbnail() {
	...
	return apply_filters(
		'emoson_can_show_post_thumbnail',
...
function emoson_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . emoson_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'emoson_continue_reading_link_excerpt' );
...
function emoson_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . emoson_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'emoson_continue_reading_link' );

if ( ! function_exists( 'emoson_post_title' ) ) {
	/**
...
add_filter( 'the_title', 'emoson_post_title' );
 ...
 * @return string
 */
function emoson_get_icon_svg( $group, $icon, $size = 24 ) {
	return Emoson_SVG_Icons::get_svg( $group, $icon, $size );
}
...
function emoson_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? emoson_get_icon_svg( 'ui', 'arrow_right' ) : emoson_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? emoson_get_icon_svg( 'ui', 'arrow_left' ) : emoson_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'emoson_change_calendar_nav_arrows' );
...
function emoson_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'emoson_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'emoson_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'emoson_generate_css' ) ) {
	...
	return emoson_generate_css( // @phpstan-ignore-line.
 ...
 *
 * @since Emoson 1.0
 * @since Emoson 1.4 Corrected parameter name for `$output`,
...
function emoson_password_form( $output, $post = 0 ) {
	...
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'emoson' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'emoson' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'emoson' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'emoson_password_form', 10, 2 );
...
add_filter( 'wp_get_attachment_image_attributes', 'emoson_get_attachment_image_attributes', 10, 3 );
